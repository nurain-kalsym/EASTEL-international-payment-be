...
public class SmsService {
    ...
    public String sendHttpGetRequest(String destAddr, String message, Boolean otp) {
        try {
            ...
            Logger.application.info(Logger.pattern, InternationalPaymentApplication.VERSION, "fullUrl", fullUrl);

            // Process the response
            if (response.getStatusCode().is2xxSuccessful()) {
                return response.getBody();
            } else {
                Logger.application.error(Logger.pattern, InternationalPaymentApplication.VERSION, "sendHttpGetRequest",
