...
public class FraudCheckClient {
    ...
    public HttpResponse firstCheckFraud(HttpServletRequest request, String transactionId) {
        ...
        Logger.application.info(Logger.pattern, EkedaiApplication.VERSION, logprefix, "Response Object succes:: ",
                getResponseBody);

        Integer status = newResponse.getStatusCodeValue();
        Logger.application.info(Logger.pattern, EkedaiApplication.VERSION, logprefix, "status: ", status);
        if (status.equals(200)) {
            Logger.application.info(Logger.pattern, EkedaiApplication.VERSION, logprefix, "api response: ",
